...
def backwards_rotation(start_assignments, dates):
    """
    start_assignments: dict {date: doctor} for first week only.
    ...
    # Πρώτα η αρχική εβδομάδα όπως την επέλεξε ο χρήστης
    first_week_dates = sorted(start_assignments.keys())
    for d in first_week_dates:
        assignments[d] = start_assignments[d]

    # Υπόλοιπες ημερομηνίες
    remaining_dates = [d for d in dates_sorted if d not in first_week_dates]

    # Ομαδοποίηση σε εβδομάδες των 7 ημερών
    weeks = [remaining_dates[i:i+7] for i in range(0, len(remaining_dates), 7)]

    for w_idx, block in enumerate(weeks):
        # +1 για να ξεκινήσει rotation από 2η εβδομάδα
        offset = ((w_idx + 1) * 2) % 7
...
def create_pdf(assignments, filename="schedule.pdf"):
    """Create PDF with date + day + doctor"""
    pdf = FPDF()
    pdf.add_page()
    ...
    for d in sorted(assignments.keys()):
        ...
        pdf.cell(60, 8, txt=f"{date_str} – {weekday_str}", border=1)
...
for d in sorted(assignments.keys()):
    st.write(f"{d.strftime('%d/%m/%Y')} – {d.strftime('%a')} → {assignments[d]}")
